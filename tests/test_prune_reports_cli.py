...
def _seed_scan_dirs(reports_dir: Path, project: str, scan_ids: list[str]) -> None:
    for scan_id in scan_ids:
        scan_dir = reports_dir / "projects" / project / "runs" / scan_id
...
def test_prune_reports_keep_and_dry_run(tmp_path: Path) -> None:
    ...
    project.mkdir()
    reports_dir = project / "CodeGauge"
    (project / ".codegauge.toml").write_text(f'report_root = "{reports_dir.as_posix()}"\n')
    ...
    assert len(list((reports_dir / "projects" / "alpha" / "runs").iterdir())) == 3
    ...
    assert len(list((reports_dir / "projects" / "alpha" / "runs").iterdir())) == 2
...
def test_prune_reports_days(tmp_path: Path) -> None:
    ...
    remaining = [path.name for path in (reports_dir / "projects" / "alpha" / "runs").iterdir()]
